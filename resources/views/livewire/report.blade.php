<div x-data="{ report: @entangle('report') }">
    <div class="relative   flex space-x-2   items-end">
        <x-native-select label="Report" wire:model="report" class="w-full">
    ...
    <div class="mt-10 relative">
        @if ($report == 1)
            <div class="">
                <div class="flex justify-between items-center relative border-b pb-2">
            ...
            <div class="">
                ...
                <div class="mt-10">
                    <div x-ref="printContainer">
                        ...
                        <div class="mt-5 text-center ">
                            <h1 class="font-bold text-xl text-gray-700">EXPENSES REPORT</h1>
                            @if ($date_from && $date_to)
                                <h1 class="text-sm text-gray-500">(
                                    {{ \Carbon\Carbon::parse($date_from)->format('F d, Y') }}
                                    {{ $date_from != $date_to ? '- ' . \Carbon\Carbon::parse($date_to)->format('F d, Y') : '' }}
                                    ...
                                    @forelse ($reports as $report)
                                        <tr>
                                            <td colspan="4" class="border-2 text-gray-700  px-3 py-1">
                                                <h1>
                                                    <span>OR #: </span>
                                                    <span class="font-bold">{{ $report->voucher_number }}</span>
                                                </h1>
                                                <h1> <span>NAME: </span>
                                                    <span class="font-bold">{{ $report->name }}</span>
                                                </h1>
                                        ...
                                        @foreach ($report->expense_category_transactions as $expense)
                                            <tr>
                                                <td class=" text-gray-700  px-3 py-1">
                                                </td>
                                                <td class=" text-gray-700  px-3 py-1">
